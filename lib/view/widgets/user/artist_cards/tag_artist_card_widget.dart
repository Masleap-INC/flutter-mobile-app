...
class TaggedArtistCardViewModel extends GetxController {
  final RxString artistId;
  final Rxn<UserModel> artist;

  TaggedArtistCardViewModel({String? artistId, UserModel? artist})
  ...
  untagThisArtist() async {
    _getTaggedUser().removeAt(_getTaggedUser().indexOf(artist.value!));
...
class TaggedArtistCardWidget extends StatelessWidget {
  final TaggedArtistCardViewModel controller;
  final VoidCallback? onTap;

  const TaggedArtistCardWidget({Key? key, required this.controller, this.onTap})
      : super(key: key);
  ...
  Widget _trailingButton() {
    return Obx(
          () => controller.isTagged.value
          ? OutlinedButton(
        onPressed: ()=> controller.untagThisArtist(),
        child: Text(
          'Remove',
        ...
        onPressed: ()=> controller.tagThisArtist(),
      ),
    );
  }
}
